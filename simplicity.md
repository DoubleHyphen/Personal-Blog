...
As soon as we move beyond those, and get into things like handles for files or threads or servers, copying is a liability, not an asset. {{{Accessing a file requires us to be certain that we're the only ones currently doing so.}}} With that in mind, the borrow checker unlocks a _serious_ super-power: _Any reference to any of those things is compiler-guaranteed to find the underlying object in a useable state._ The amount of complexity this eliminates cannot be overstated. For instance, let's translate the channel example above into Rust:
